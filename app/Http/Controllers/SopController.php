...
class SopController extends Controller
{
   ...
   public function delete($id){
    ...
    {   
        ...
        $sop = Sop::where('publish','1')->orderBy('updated_at','DESC')->simplePaginate(15)->onEachSide(1);
        $category = Category::all();
    ...
    public function getSearch(Request $request){
        ...
        if($request->category == 'all'){
            $sop = Sop::leftJoin('sop_relation_category','sop_relation_category.id_sop','sop.id')->join('category','category.id','sop_relation_category.id_category')->where('sop.slug','like','%'.$request->search.'%')->orderBy('sop.updated_at','DESC')->where('sop.publish','1')->select('sop.*')->groupBy('sop.id')->simplePaginate(15)->onEachSide(1);
        }else{
            $sop = Sop::leftJoin('sop_relation_category','sop_relation_category.id_sop','sop.id')->join('category','category.id','sop_relation_category.id_category')->where('sop.slug','like','%'.$request->search.'%')->orderBy('sop.updated_at','DESC')->where('sop_relation_category.id_category',$request->category)->where('sop.publish','1')->select('sop.*')->groupBy('sop.id')->simplePaginate(15)->onEachSide(1);
