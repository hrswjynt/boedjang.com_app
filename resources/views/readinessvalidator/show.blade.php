						...
						<form method="POST" action="{{ route('readinessvalidator.update', $matrixHeader->id) }}"
							...
							<div class="container-fluid mt-3">
								<div class="row">
									...
									<div class="col-9">
										<input type="text" class="form-control mb-3" value="{{ $matrixHeader->dataAtasan->atasan_name }}" readonly>
									</div>
									...
									<div class="col-12">
										<div class="table-responsive">
											<table id="table-{{ str_replace(' ', '-', strtolower($matrixHeader->dataBagian->nama)) }}" class="table">
												<thead>
													<tr>
														<th>No</th>
														<th>Kode</th>
												...
												<tbody>
													@foreach ($matrixHeader->matrix as $i => $matrix)
														<tr>
															...
															<td>
																<input type="checkbox" name="staff[]" value="{{ $matrix->id }}"
																	@if ($matrix->staff_valid) checked @endif disabled>
															</td>
															<td>
																<input type="checkbox" name="valid[]" value="{{ $matrix->id }}"
																	@if ($matrix->atasan_valid) checked @endif disabled>
															</td>
															<td>
																<input type="checkbox" name="hc[]" value="{{ $matrix->id }}"
																	@if ($matrix->validator) checked @endif @if (!$matrix->atasan_valid_date) disabled @endif>
															</td>
														</tr>
													@endforeach
												</tbody>
