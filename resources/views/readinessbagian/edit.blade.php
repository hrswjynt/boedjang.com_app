            ...
            <div class="col-md-12">
                <div class="card shadow mb-4">
                    <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                        <h6><b>Edit Bagian Readiness</b></h6>
                    ...
                    <div class="card-body">
                        ...
                        @endif
                        <form method="POST" action="{{ route('readinessbagian.update', $bagian->id) }}"
                            id="readinessbagian_form">
                            @csrf
                            @method('PUT')
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $("#btn-submit").click(function() {
                swal({
                        title: "Apakah anda yakin akan mengubah data bagian readiness?",
                    ...
                    .then((willDelete) => {
                        if (willDelete) {
                            ...
                            swal("Proses Pengubahan Data Bagian Readiness Dibatalkan!", {
                                icon: "error",
                            });
                        }
                    });
                ...
                @foreach ($kategori as $k)
                    {
                        id: '{{ $k->id }}',
                        nama: '{{ $k->nama }}',
                        selected: {{ $k->id == $bagian->readiness_kategori ? 'true' : 'false' }},
                        jenis: [
                            @foreach ($k->jenis as $j)
                                {
                                    id: '{{ $j->id }}',
                                    nama: '{{ $j->nama }}',
                                    selected: {{ $j->id == $bagian->readiness_jenis ? 'true' : 'false' }},
                                },
                            @endforeach
                        ]
                    },
